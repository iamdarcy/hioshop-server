...
module.exports = class extends Base {
    ...
    async dataAction() {
        ...
        let info = await this.model('region').where({parent_id: parentId}).getField('id,name');
    ...
    async codeAction() {
        ...
        console.log(city);
        console.log(country);
        let provinceInfo = await this.model('region').where({name: province}).field('id').find();
        let province_id = provinceInfo.id;
        let cityInfo = await this.model('region').where({name: city}).field('id').find();
        let city_id = cityInfo.id;
        let countryInfo = await this.model('region').where({name: country}).field('id').find();
        let country_id = countryInfo.id;
        console.log(province_id + ':' + city_id + ':' + country_id);
        let data = {
